...
private:
  ...
  void DoRunFifoTestWithAck(Ptr<FifoQueueDisc> q, uint32_t qSize, uint32_t pktSize);
};
...
void
FifoQueueDiscTestCase::DoRunFifoTestWithAck (Ptr<FifoQueueDisc> q, uint32_t qSize, uint32_t pktSize)
{
  std::vector<uint64_t> uids;
  Ptr<Packet> p;
  Ptr<QueueDiscItem> item;
  ...
  uids.push_back (p->GetUid ());
  q->Enqueue (Create<FifoQueueDiscTestItem> (p, dest));
  NS_TEST_EXPECT_MSG_EQ (q->GetCurrentSize ().GetValue (), 1, "There should be " << 1 << " packet(s) in there");
  std::cout<<q->GetCurrentSize ().GetValue () << " Elementsssss" << "\n";
  ...
  uids.push_back (p->GetUid ());
  q->Enqueue (Create<FifoQueueDiscTestItem> (p, dest));
  NS_TEST_EXPECT_MSG_EQ (q->GetCurrentSize ().GetValue (), 2, "There should be " << 2 << " packet(s) in there");
  std::cout<<q->GetCurrentSize ().GetValue () << " lementsss" << "\n";
...
FifoQueueDiscTestCase::RunFifoTest (QueueSizeUnit mode)
{
  ...
  NS_TEST_EXPECT_MSG_EQ (queue->GetNInternalQueues (), 0, "Verify that the queue disc has no internal queue");
  queue->Initialize ();
  DoRunFifoTestWithAck (queue, numPackets*modeSize, pktSize);
